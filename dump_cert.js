...
Java.perform(function () {
    ...
    hookKeystoreLoad(true);
    hookKeystoreLoadStream(true);
    hookKeystoreGetKey();
...
function hookSSLContentInit() {
    var SSLContentInit = Java.use('javax.net.ssl.SSLContent').init.overload("javax.net.ssl.KeyManager[]", "javax.net.ssl.TrustManager[]", "java.security.SecureRandom");
    SSLContentInit.implementation = function (keyManagers, trustManagers, secureRandom) {
        console.log("SSLContext initialized");
        if (trustManagers) {
            for (var i = 0; i < trustManagers.length; i++) {
                console.log("TrustManager: " + trustManagers[i].toString());
            }
        }
...
function hookKeystoreLoadStream(dump) {
    ...
    keyStoreLoadStream.implementation = function (stream, charArray) {
        ...
        console.log("[Keystore.load(InputStream, char[])]: keystoreType: " + this.getType() + ", password: '" + charArrayToString(charArray) + "', inputSteam: " + stream + ", hexString: " + hexString);
